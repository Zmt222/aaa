<template>
  <div>
    <Modal :value="checkModal" title="协查请求审核窗口" @on-cancel="cancel">
        ...
        <Row type="flex" justify="center">
          ...
          <FormItem label="跟进人" prop="followCode">
            <Select v-model="checkForm.followCode" @on-change="changeFollowCode">
              <Option v-for="item in followUsersArr" :value="item.userCode" :key="item.userCode">{{item.userName
                }}</Option>
            </Select>
          </FormItem>
      ...
      pass() {
        this.$refs.checkForm.validate(valid => {
          if (valid) {
            ...
            this.$http
              .post(
                this.$store.state.api.flowPathController.pass(),
