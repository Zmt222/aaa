<template>
  <div>
    <Form :model="createForm" ref="createForm" :rules="createFormRules" :label-width="140">
      <Row>
        <Col class="col" offset="5">
          <FormItem class="assistItem">
            <RadioGroup v-model="createForm.assist" class="assistBtn">
              <Radio class="radio" label="0">普通请求</Radio>
              <Radio label="1">紧急请求</Radio>
            </RadioGroup>
          </FormItem>
        </Col>
      </Row>
      ...
      <Row>
        ...
        <Col span="7">
          <FormItem label="升级响应" prop="clearanceStatus">
    ...
    data() {
      ...
      return {
        ...
        createFormRules: {
          ...
          clearanceStatus: [
            { required: true, message: "升级响应不能为空", trigger: "change" }
          ],
          followPostil: [{ required: true, message: "留言不能为空", max: 100, trigger: "blur" }]
        },
        dateOptions: {
          afterTody: {
            disabledDate(date) {
            ...
            this.$http
              ...
              .then(res => {
                ...
                if (res.data.result) {
                  ...
                  //传递参数
                  // let data = {
                  //   fileTrueName: this.fileTrueName,
                  //   fileServerName: this.fileServerName
                  // }
                  // this.$emit("newParameter",data)
                  // if (res.data.data === "pending") {
                    this.$store.commit("navigateTo", "/assistindex/findall");
